...
const CustomTooltip = ({ active, payload, label, units }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-[color:var(--bg-input)] p-3 rounded-lg border border-[color:var(--border-color)] shadow-lg text-xs backdrop-blur-md z-50">
        ...
        {payload.map((p: any) => (
          <div key={p.dataKey} style={{ color: p.color, display: p.value === undefined ? 'none' : 'block' }}>
            {p.dataKey === 'temperature' && `Temp: ${temp}° ± ${deviation}° (${data.confidence || 100}%)`}
...
const WeatherChart: React.FC<WeatherChartProps> = ({ hourlyData, dailyData, units, className }) => {
  const [isMounted, setIsMounted] = useState(false);
  const [view, setView] = useState<'hourly' | 'daily'>('hourly');
  const [visibility, setVisibility] = useState<VisibilityState>({
    temperature: true, feelsLike: false, precipitation: true, humidity: false,
    windSpeed: false, uvIndex: false, historicalAvgTemp: false, pressure: false,
  ...
  return (
    <div className={`w-full bg-[color:var(--bg-card)] rounded-lg p-4 border border-[color:var(--border-color)] backdrop-blur-lg flex flex-col ${className || 'h-[28rem] mt-4'}`}>
      <div className="flex justify-between items-center mb-2 flex-shrink-0">
        <h3 className="text-xs font-semibold text-[color:var(--text-secondary)] uppercase tracking-wider">Forecast Analysis</h3>
        ...
        {isMounted && (
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={processedData} margin={{ top: 10, right: 10, left: -20, bottom: 5 }}>
              ...
              <XAxis dataKey="time" tick={{ fill: 'var(--text-secondary)', fontSize: 10 }} axisLine={false} tickLine={false} interval="preserveStartEnd" minTickGap={20} />
              <YAxis yAxisId="temp" tick={{ fill: 'var(--text-secondary)', fontSize: 10 }} axisLine={false} tickLine={false} unit={tempUnit} width={30} />
              <YAxis yAxisId="percent" orientation="right" tick={{ fill: 'var(--text-secondary)', fontSize: 10 }} axisLine={false} tickLine={false} unit="%" width={30} />
              ...
              <Bar yAxisId="percent" dataKey="precipitation" name="Precip %" barSize={8} opacity={0.6} hide={!visibility.precipitation}>
